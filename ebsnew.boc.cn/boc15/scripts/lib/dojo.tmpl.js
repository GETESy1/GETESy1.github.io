/*! BUILD: 2014-04-24 */ ! function(a) {
    ...
    function c(b, e, f) {
        ...
        return e ? h : (1 == h.length && h[0].join && (h = h[0]), h = h.join(""), h.replace(/^\s*([^<\s][^<]*)?(<[\w\W]+>)([^>]*[^>\s])?\s*$/, function(b, c, e, f) {
    ...
    function e(b) {
        return new Function("$", "$item", "var $=$,call,_=[],$data=$item.data;with($data){_.push('" + a.trim(b).replace(/([\\'])/g, "\\$1").replace(/[\r\t\n]/g, " ").replace(/\$\{([^\}]*)\}/g, "{{= $1}}").replace(/\{\{(\/?)(\w+|.)(?:\(((?:[^\}]|\}(?!\}))*?)?\))?(?:\s+(.*?)?)?(\(((?:[^\}]|\}(?!\}))*?)\))?\s*\}\}/g, function(b, c, d, e, f, h, i) {
            var j, k, l, m = a.tmpl.tag[d];
            if (!m) throw "Template command not found: " + d;
            return j = m._default || [], h && !/\w$/.test(f) && (f += h, h = ""), f ? (f = g(f), i = i ? "," + g(i) + ")" : h ? ")" : "", k = h ? f.indexOf(".") > -1 ? f + h : "(" + f + ").call($data" + i : f, l = h ? k : "(typeof(" + f + ")==='function'?(" + f + ").call($item):(" + f + "))") : l = k = j.$1 || "null", e = g(e), "');" + m[c ? "close" : "open"].split("$notnull_1").join(f ? "typeof(" + f + ")!=='undefined' && (" + f + ")!=null" : "true").split("$1a").join(l).split("$1").join(k).split("$2").join(e ? e.replace(/\s*([^\(]+)\s*(\((.*?)\))?/g, function(a, b, c, d) {
    ...
    function i(c) {
        function d(c) {
            ...
            if (g) {
                for (f = g; f && f.key != i && 0 != f.key;) f.nodes.push(c), f = f.parent;
    ...
    a.each({
        ...
        a.fn[b] = function(b) {
            var d;
            return o = s || {}, this.targets[c](b), d = o, o = null, a.tmpl.complete(d), this
        }
    }), a.fn.extend({
        tmpl: function(b, c, d) {
        ...
        tmplItem: function() {
            return a.tmplItem(this.get(0))
        },
        tmplElement: function() {
            return a.tmplElement(this.get(0))
        },
        template: function(b) {
            return a.template(b, this.get(0))
        },
        ...
        tmpl: function(d, e, g, h) {
            var i, j = !h;
            if (j) h = u, "function" != typeof d && (d = a.template[d] || a.template(null, d)), t = {};
        ...
        tmplElement: function(b) {
            for (b instanceof a && (b = b[0]); b && 1 === b.nodeType && !a.data(b, "tmplItem") && (b = b.parentNode););
            return b
        },
        template: function(b, c) {
            if (c) {
                if ("string" == typeof c ? c = e(c) : c instanceof a && (c = c[0] || {}), c.nodeType) {
                    var d = 11 == c.nodeType ? c.childNodes[0].innerHTML : c.innerHTML;
                    c = a.data(c, "tmpl", e(d))
                }
                return "string" == typeof b ? a.template[b] = c : c
            }
            return b ? "string" != typeof b ? a.template(null, b) : a.template[b] || a.template(null, r.test(b) ? b : a(b)) : null
        },
        encode: function(a) {
